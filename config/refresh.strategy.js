...
function RefreshStrategy(options, verify) {
  ...
  this._secretOrKey = options.refreshSecret;
...
util.inherits(RefreshStrategy, passport.Strategy);
 ...
 */
RefreshStrategy.JwtVerifier = function(token, secretOrKey, options, callback) {
 ...
 */
RefreshStrategy.prototype.authenticate = function(req, options) {
  var self = this;
  ...
  if (!token) {
    return self.fail(new Error("No refresh token"));
  }

  // Verify the JWT
  RefreshStrategy.JwtVerifier(token, this._secretOrKey, this._verifOpts, function(jwt_err, payload) {
